...
public abstract class SpriteCanvas extends Canvas {
	...
	public void createFrameBuffer(int fw, int fh) {
		spriteImage = Image.createImage(width, height);
	...
	public void drawFrameBuffer(int tx, int ty) {
		// ?
		platformImage.getGraphics().drawImage(spriteImage, tx, ty, 0);
		Mobile.getPlatform().flushGraphics(platformImage, 0, 0, platformImage.width, platformImage.height);
		((CanvasGraphics)graphics).clearRect(0, 0, spriteImage.width, spriteImage.height);
	}
