...
public class FigureImpl {
	...
	private void sortPolygons() {
		...
		for (Model.Polygon p : polygonsT) {
			...
			if (length != 0) {
				model.hasBlendModeT[p.blendMode >> 1] = true;
			}
		}
		...
		for (Model.Polygon p : polygonsC) {
			...
			if (length != 0) {
				model.hasBlendModeC[p.blendMode >> 1] = true;
			}
		}
	}
	...
	synchronized void prepareBuffers() {
		if (model.verticesNormalsBuffer == null) {
			model.verticesNormalsBuffer = new float[model.vertexArrayCapacity * 2];
		}

		Utils.fillVerticesNormalsBuffer(model.verticesNormalsBuffer,
			 model.vertices, model.originalNormals != null ? model.normals : null, model.indices);
	}
	...
	synchronized void fillBuffers(float[] verticesNormalsBuffer) {
		Utils.fillVerticesNormalsBuffer(verticesNormalsBuffer,
		model.vertices, model.originalNormals != null ? model.normals : null, model.indices);
	}
}
