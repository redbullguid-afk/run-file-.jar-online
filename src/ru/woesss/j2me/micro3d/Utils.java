...
public class Utils {
    ...
    public static void fillVerticesNormalsBuffer(float[] buffer, float[] vertices, float[] normals, int[] indices) {
		int bufPos = 0;
        ...
        for (int index : indices) {
			...
			if (normals != null) {
				buffer[bufPos++] = normals[index * 3];
				buffer[bufPos++] = normals[index * 3 + 1];
				buffer[bufPos++] = normals[index * 3 + 2];
			} else {
				buffer[bufPos++] = buffer[bufPos++] = buffer[bufPos++] = 0;
			}
        }
    }
