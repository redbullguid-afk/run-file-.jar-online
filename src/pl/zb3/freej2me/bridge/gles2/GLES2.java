...
public class GLES2 {
    ...
    private static Object program = 0;
    // note they keep uniform handles even if the program is gone..
    // weak refs?
    private static Map<Object, Map<Object, Float>> uniform1fCache = new HashMap<>();
    ...
    private static Object boundTexture2D = 0;

    private static Set<Integer> enabledArrays = new HashSet<>();
    ...
    private static Map<Object, Map<Integer, Integer>> texParamI = new HashMap<>();
    private static Map<Object, Map<Integer, Float>> texParamF = new HashMap<>();
    ...
    public static void uniform1f(Object loc, float f1) {
        ...
        Map<Object, Float> programCache = uniform1fCache.get(program);
        ...
        if (cached != null && cached == f1) {
            return;
        }

        programCache.put(loc, f1);
    ...
    public static void uniform1i(Object loc, int i1) {
        // check and write cache
        Map<Object, Integer> programCache = uniform1iCache.get(program);
        if (programCache == null) {
            programCache = new HashMap<>();
            uniform1iCache.put(program, programCache);
        }

        Integer cached = programCache.get(loc);
        if (cached != null && cached == i1) {
            return;
        }

        programCache.put(loc, i1);
    ...
    public static void uniform2f(Object loc, float f1, float f2) {
        // check and write cache
        Map<Object, float[]> programCache = uniform2fCache.get(program);
        if (programCache == null) {
            programCache = new HashMap<>();
    ...
    public static void uniform4f(Object loc, float f1, float f2, float f3, float f4) {
        // check and write cache
        Map<Object, float[]> programCache = uniform4fCache.get(program);
        if (programCache == null) {
            programCache = new HashMap<>();
    ...
    public static void uniformMatrix3fv(Object loc, boolean transpose, float[] fa) {
        // check cache
        Map<Object, MatrixEntry> programCache = uniformMatrix3Cache.get(program);
        if (programCache == null) {
            programCache = new HashMap<>();
            uniformMatrix3Cache.put(program, programCache);
        }

        MatrixEntry cached = programCache.get(loc);
        if (cached != null) {
            if (cached.transposed == transpose && Arrays.equals(fa, cached.elements)) {
                return;
            }
    ...
    public static void uniformMatrix4fv(Object loc, boolean transpose, float[] fa) {
        // check cache
        Map<Object, MatrixEntry> programCache = uniformMatrix4Cache.get(program);
        if (programCache == null) {
            programCache = new HashMap<>();
    ...
    public static void texParameterf(int target, int pname, float param) {
        if (target == Constants.GL_TEXTURE_2D) {
            Map<Integer, Float> paramCache = texParamF.get(boundTexture2D);
            if (paramCache == null) {
                paramCache = new HashMap<>();
                texParamF.put(boundTexture2D, paramCache);
            }

            if (paramCache.get(pname) != null && paramCache.get(pname) == param) {
                return;
            }

            paramCache.put(pname, param);
        }
    ...
    public static void texParameteri(int target, int pname, int param) {
        if (target == Constants.GL_TEXTURE_2D) {
            Map<Integer, Integer> paramCache = texParamI.get(boundTexture2D);
            if (paramCache == null) {
                paramCache = new HashMap<>();
    ...
    public static void disable(int flag) {
        if (!enabledCache.contains(flag)) { return; }
        enable(handle, flag);
        enabledCache.remove(flag);
        disable(handle, flag);
        checkException();
    }
    ...
    public static void disableVertexAttribArray(int index) {
        if (boundVertexArray == null && !enabledArrays.contains(index)) {return;}
